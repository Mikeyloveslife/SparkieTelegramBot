...
async def buy_tokens_ukr(message: Message, state: FSMContext):
  ...
  if current_state == Form.in_check_balance_ukr.state:
    ...
    await bot.send_message(message.from_user.id, f'Ваш баланс токенів: {token_balance}.', reply_markup=buy_tokens_kb)
...
async def check_balance_ukr(message: Message):
  ...
  user_id = message.from_user.id
  token_balance = get_token_balance(user_id)
  await bot.send_message(message.from_user.id, f'Ваш баланс токенів: {token_balance}', reply_markup=check_balance_kb)
