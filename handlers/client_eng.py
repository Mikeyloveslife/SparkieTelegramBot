...
async def in_text_davinci_003_eng(message: Message, state: FSMContext):
  if message.text == '\u2B05':
    ...
    await Form.in_choose_model.set()
    await bot.send_message(message.from_user.id, f"Your current model is {get_model(message.from_user.id)}.", reply_markup=return_kb)
    await bot.send_message(message.from_user.id, f"Choose one of the supported models:", reply_markup=model_kb)
  else:
...
async def in_temperature_inline_eng(callback_query: CallbackQuery, state: FSMContext):
  ...
  if 0 <= data <= 1:
    ...
    await Form.in_model_settings.set()
    await bot.send_message(callback_query.from_user.id, f"Temperature has been set to {data}.", reply_markup=model_settings_kb)

    
              ### Return button for In_temperature ###
#@dp.message_handler((Text(equals=["\u2B05", "\uFF1F"])) | (lambda message: 0 <= float(message.text) <= 1), state=Form.in_temperature)
async def in_temperature_reply_eng(message: Message, state: FSMContext):
  if message.text == "\u2B05":
    ...
    await Form.in_model_settings.set()
    await bot.send_message(message.from_user.id, f"Temperature has been set to {float(message.text)}.", reply_markup=model_settings_kb)
...
async def in_choose_model_inline_eng(callback_query: CallbackQuery, state: FSMContext):
  ...
  if data == "gpt-3.5-turbo":
    ...
    await Form.in_gpt_turbo.set()
    await bot.send_message(callback_query.from_user.id, f"The model has been changed to {data}. You can keep chatting.", reply_markup=in_gpt_turbo_kb)
  elif data == 'text-davinci-003':
    update_model(callback_query.from_user.id, data)
    await Form.in_text_davinci_003.set()
    await bot.send_message(callback_query.from_user.id, f"The model has been changed to {data}. You can keep chatting.", reply_markup=in_text_davinci_003_kb)
